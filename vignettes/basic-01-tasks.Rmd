...
In `mlrng`, each row has a unique identifier (row name) which can be either `integer` or `character.`
...
In `mlrng`, columns (and rows) can have specific roles w.r.t. the learning task.
...
In the following we do not want to learn on neither the primary key (which is already handled by `mlrng`) nor the new column `rn` with the row names.
...
2. Role `"ignore"`: Rows that are simply ignored
...
A task can have different data backends.
The iris example task (like all predefined toy task) uses a simple `data.table` backend with a rather minimal API:
```{r}
task = mlr.tasks$get("iris")
task$backend
```
The abstraction of the backend allows to transparently transparently work with real data bases instead of in-memory data.
This is illustrated in the following.

As an alternative to `data.table`, it is also possible to use a `dplyr` backend to construct a task.
First, we create a small SQLite data base and create a `tbl` object for it.
...
con <- DBI::dbConnect(RSQLite::SQLite(), path = tempfile())
copy_to(con, data, "iris")
tbl = tbl(con, "iris")
print(tbl)
```

```{r}
b = mlrng:::BackendDplyr$new(data = tbl, primary.key = "id")
```

This backend can be passed to a task during construction as argument `data`:

```{r}
task = TaskClassif$new(id = "iris", data = b, target = "Species")
```
We have now created a task which transparently queries a data base for us.
All previously introduced task operations work out of the box.

Note that it is also possible to restrict the task to specific columns or rows.
Here, we use the `tbl` object to create a binary classification task (`"setosa"` vs. `"versicolor"`):
```{r}
b = mlrng:::BackendDplyr$new(
  data = tbl %>% filter(Species %in% c("setosa", "versicolor")),
  primary.key = "id")
task = TaskClassif$new(id = "iris", data = b, target = "Species")
task$classes
task$nrow
```
All functions supported by `dplyr`/`dbplyr` are supported this way.
For example, you can also mutate columns with transformations, convert their type, or impute missing values.
```{r}
b = mlrng:::BackendDplyr$new(
  data = tbl %>%
