...
resample = function(task, learner, resampling, measures) {
  ...
  if (resampling$is.instantiated) {
    instance = resampling$clone() # FIXME: some checks required
  } else {
    instance = resampling$clone()$instantiate(task)
  }
  n = instance$iters
  ...
  res = future.apply::future_lapply(seq_len(n), function(i, task, learner, instance, measures) {
    ...
    mlrng:::runExperiment(task = task, learner = learner,  train.set = train.set, test.set = test.set, measures = measures)
  }, future.globals = FALSE, future.packages = "mlrng", task = task, learner = learner, instance = instance, measures = measures)


  res = combineExperiments(res)
  res[, c("task", "learner", "resampling", "iteration") := list(list(task), list(learner), list(resampling), seq_len(n))]
