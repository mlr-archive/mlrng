...
mlr.learners$add(
  ...
  train = function(task, subset, ...) {
    ...
    ctrl = learnerArgsToControl(party::cforest_control, ntree = self$par.vals$ntree, mtry = self$par.vals$mtry, 
      replace = self$par.vals$replace, fraction = self$par.vals$fraction, trace = self$par.vals$trace, teststat = self$par.vals$teststat, 
      testtype = self$par.vals$testtype, mincriterion = self$par.vals$mincriterion,
      minsplit = self$par.vals$minsplit, minbucket = self$par.vals$minbucket, stump = self$par.vals$stump, nresample = self$par.vals$nresample, 
      maxsurrogate = self$par.vals$maxsurrogate, maxdepth = self$par.vals$maxdepth, savesplitstats = self$par.vals$savesplitstats)
