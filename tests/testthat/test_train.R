...
test_that("Training with default options", {
  ...
  expect_character(mod$train.set, len = 50L, any.missing = FALSE, unique = TRUE)
  expect_result(mod)
  p = predict(mod)
...
test_that("Training on subsets", {
  mod = train(task, lrn, subset = 1:40)
  expect_is(mod, "TrainResult")
  expect_true(mod$train.success)
  expect_character(mod$train.set, len = 40L, any.missing = FALSE, unique = TRUE)
  expect_result(mod)
  p = predict(mod, subset = 41:50)
  expect_is(p, "PredictResult")
  expect_character(p$test.set, len = 10L, any.missing = FALSE, unique = TRUE)
  expect_subset(p$predicted, task$classes)
  expect_result(mod)
})
